...
class Camas extends Config{
    ...
    public function AjaxGestionCamas() {
        if($this->input->post('area')==''){
            ...
            $Mantenimiento= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Mantenimiento FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_status='En Mantenimiento' $Condition")[0]['Mantenimiento'];
                $Limpieza= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS Limpieza FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_status='En Limpieza' $Condition")[0]['Limpieza'];
                $Ocupadas= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS ocupadas FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_status='Ocupado' $Condition")[0]['ocupadas'];
                $Disponible= $this->config_mdl->_query("SELECT COUNT(os_camas.cama_id) AS disponible FROM os_camas, os_areas
                WHERE os_camas.area_id=os_areas.area_id AND os_camas.cama_status='Disponible' $Condition")[0]['disponible'];
    ...
    public function ObtenerPisos() {
        if($this->UMAE_AREA=='Piso 2 Oriente'){
            return 1;
        }if($this->UMAE_AREA=='Piso 2 Poniente'){
            return 2;
        }if($this->UMAE_AREA=='Piso 3 Oriente'){
            return 3;
        }if($this->UMAE_AREA=='Piso 3 Poniente'){
            return 4;
        }if($this->UMAE_AREA=='Piso 4 Oriente'){
            return 5;
        }if($this->UMAE_AREA=='Piso 4 Poniente'){
            return 6;
        }if($this->UMAE_AREA=='Piso 5 Oriente'){
            return 7;
        }if($this->UMAE_AREA=='Piso 5 Poniente'){
            return 8;
        }if($this->UMAE_AREA=='Piso 6 Oriente'){
            return 9;
        }if($this->UMAE_AREA=='Piso 6 Poniente'){
            return 10;
        }
